...
<div class="flex items-center justify-between gap-3 mt-2 px-1 flex-wrap">
  ...
  <div class="flex items-center gap-1">
    <button <?= $curPage <= 1 ? 'disabled' : '' ?> onclick="muatDaftarTrip(<?= $curPage - 1 ?>)" class="px-3 py-1.5 text-xs rounded-md <?= $curPage <= 1 ? 'text-slate-400 cursor-not-allowed' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' ?>">Sebelumnya</button>
    <span class="px-2 text-xs text-slate-500 dark:text-slate-400">Hal. <?= $curPage ?> / <?= $totalPages ?></span>
    <button <?= $curPage >= $totalPages ? 'disabled' : '' ?> onclick="muatDaftarTrip(<?= $curPage + 1 ?>)" class="px-3 py-1.5 text-xs rounded-md <?= $curPage >= $totalPages ? 'text-slate-400 cursor-not-allowed' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' ?>">Berikutnya</button>
  </div>
</div>
<?php endif; ?>
