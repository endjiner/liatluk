...
<div class="card mb-4">
  <div class="card-header flex items-center justify-between gap-2 flex-wrap">
    ...
    <div class="flex items-center gap-1.5 flex-wrap">
      <button type="button" class="btn btn-outline btn-xs sm:btn-sm gap-1.5 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 border-emerald-300 dark:border-emerald-800" onclick="exportDanaTaktis('excel')">
        <?= iconsax('document-text', 'w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600') ?> Export Excel
      </button>
      <button type="button" class="btn btn-outline btn-xs sm:btn-sm gap-1.5 text-rose-700 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 border-rose-300 dark:border-rose-800" onclick="exportDanaTaktis('pdf')">
        <?= iconsax('document-text', 'w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-600') ?> Export PDF
      </button>
    </div>
  </div>
...
/* Summary */
function dtAdmUpdateSummary(summary, rows) {
  if (summary) {
    document.getElementById('dt-sum-uh').textContent     = rupiah(summary.sum_uang_harian);
    document.getElementById('dt-sum-spj').textContent    = rupiah(summary.sum_total_spj);
    document.getElementById('dt-sum-taktis').textContent = rupiah(summary.sum_dana_taktis);
    document.getElementById('dt-sum-belum').textContent  = rupiah(summary.sum_belum_dibayar);
    return;
  }
...
async function muatDaftarDanaTaktis(page) {
  ...
  try {
    ...
    dtAdmUpdateSummary(json.summary, json.data);
...
// ── Export Dana Taktis (Excel / PDF) ───────────────────────────────────────
function exportDanaTaktis(format) {
...
window.exportDanaTaktis = exportDanaTaktis;
